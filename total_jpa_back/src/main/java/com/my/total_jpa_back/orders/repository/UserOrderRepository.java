...
public interface UserOrderRepository extends JpaRepository<UserOrder, Long> {
    // userId로 조회
    // DTO 생성자의 매개변수 순서(id, name, price, status, u.name, email)와 완벽히 일치하는 쿼리입니다.
    @Query("select new com.my.total_jpa_back.orders.dto.OrderResponse(o.id, o.productName, o.price, o.status, u.name, u.email) from UserOrder o join o.user u where u.id = :userId")
...
// 4. 최신순 정렬
    ...
    @Query("select new com.my.total_jpa_back.orders.dto.OrderResponse(o.id, o.productName, o.price, o.status, u.name, u.email) from UserOrder o join o.user u where o.status = :status")
    List<OrderResponse> findOrderStatusResponse(@Param("status") OrderStatus status);

    @Query("select new com.my.total_jpa_back.orders.dto.OrderResponse(o.id, o.productName, o.price, o.status, u.name, u.email) from UserOrder o join o.user u")
